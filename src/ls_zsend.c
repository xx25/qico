...
*/
/*
 * $Id: ls_zsend.c,v 1.4 2005/03/31 19:40:38 mitry Exp $
 *
 * $Log: ls_zsend.c,v $
...
int ls_zsendsinit(char *attstr)
{
    ...
    DEBUG(('Z',2,"ls_zsendsinit: '%s'",attstr?attstr:"(null)"));

    if (attstr) {
        if (strlen(attstr) > LSZ_MAXATTNLEN-1) {
            attstr[LSZ_MAXATTNLEN-1] = '\x00';
        }
    ...
    do {
        if(retransmit) {
            DEBUG(('Z',3,"ls_zsendsinit: resend ZSINIT"));
            ...
            if((rc=ls_zsendbhdr(ZSINIT,4,ls_txHdr))<0) {
                return rc;
            }
            if((rc=ls_zsenddata(txbuf,l,ZCRCW))) {
                return rc;
            }
            retransmit = 0;
            trys++;
        ...
        case ZRINIT:		/* Skip it */
            DEBUG(('Z',3,"ls_zsendsinit: ZRINIT"));
            break;
        case ZACK:			/* Ok */
            DEBUG(('Z',3,"ls_zsendsinit: ZACK"));
            return LSZ_OK;
        case ZCHALLENGE:	/* Return number to peer, he is paranoid */
            DEBUG(('Z',3,"ls_zsendsinit: CHALLENGE"));
            ls_storelong(ls_txHdr,ls_fetchlong(ls_rxHdr));
            if((rc=ls_zsendhhdr(ZACK,4,ls_txHdr))<0) {
                return rc;
            }
            break;
        case LSZ_BADCRC:
        case ZCOMMAND:
            DEBUG(('Z',3,"ls_zsendsinit: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
            ls_storelong(ls_txHdr,0);
            if((rc=ls_zsendhhdr(ZNAK,4,ls_txHdr))<0) {
                return rc;
            }
            break;
        ...
        default:
            /*
            DEBUG(('Z',3,"ls_zsendsinit: something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zsendsinit: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            break;
    ...
    /*
    DEBUG(('Z',2,"ls_zsendsinit: timeout or something other: %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zsendsinit: timeout or something other: rc=%ld",rc));
    return rc;
}
...
int ls_zinitsender(int protocol, int baud, int window, char *attstr)
{
    ...
    DEBUG(('Z',2,"ls_zinitsender: %08x, %d baud, %d bytes",protocol,baud,window));
    ...
    do {
        if(retransmit) {
            ...
            DEBUG(('Z',3,"ls_zinitsender: resend ZRQINIT"));
            ls_storelong(ls_txHdr,0L);
            if((rc=ls_zsendhhdr(ZRQINIT,4,ls_txHdr))<0) {
                return rc;
            }
            retransmit = 0;
            trys++;
        ...
        case ZRINIT:			/* Ok, We got RINIT! */
            ...
            if(!(rxOptions&LSZ_RXCANRLE)) {
                ls_Protocol&=(~LSZ_OPTRLE);
            }
            /* Strip CRC32 from ls_Protocol, if peer could not use CRC32 */
            if(!(rxOptions&LSZ_RXCANFC32)) {
                ls_Protocol&=(~LSZ_OPTCRC32);
            }
            /* Set EscapeAll if peer want it */
            if(rxOptions&LSZ_RXWNTESCCTL) {
                ls_Protocol|=LSZ_OPTESCAPEALL;
            }
            /* Strip VHeaders from ls_Protocol, if peer could not use VHDR */
            if(!(rxOptions&LSZ_RXCANVHDR)) {
                ls_Protocol&=(~LSZ_OPTVHDR);
            }
            ...
            if(window && (!ls_txWinSize || ls_txWinSize > window)) {
                ls_txWinSize = window;
            }

            DEBUG(('Z',3,"ls_zinitsender: RINIT OK, effproto: %08x, winsize: %d",ls_Protocol,ls_txWinSize));
            ...
            if(ls_txWinSize && ls_MaxBlockSize>ls_txWinSize) {
                ...
                /*ls_MaxBlockSize >>= 1;*/
                if(ls_MaxBlockSize<32) {
                    ls_txWinSize=ls_MaxBlockSize=32;
                }
            }

            if(baud<2400) {
                ls_txCurBlockSize = 256;
            } else if(baud>=2400 && baud<4800) {
                ls_txCurBlockSize = 512;
            } else {
                ls_txCurBlockSize = 1024;
            }

            if(ls_Protocol&LSZ_OPTZEDZAP) {
                if(baud>=7200 && baud<9600) {
                    ls_txCurBlockSize = 2048;
                } else if(baud>=9600 && baud<14400) {
                    ls_txCurBlockSize = 4096;
                } else if(baud>=14400) {
                    ls_txCurBlockSize = 8192;
                }
            }
            if(ls_txCurBlockSize>ls_MaxBlockSize) {
                ls_txCurBlockSize=ls_MaxBlockSize;
            }
            DEBUG(('Z',3,"ls_zinitsender: Block sizes: %d, %d",ls_MaxBlockSize,ls_txCurBlockSize));

            /* Allocate memory for send buffer */
            txbuf = xmalloc((ls_MaxBlockSize+16));
            if ( txbuf == NULL ) {
                return LSZ_ERROR;
            }
        ...
        case ZCHALLENGE:	/* Return number to peer, he is paranoid */
            DEBUG(('Z',3,"ls_zinitsender: CHALLENGE"));
            ls_storelong(ls_txHdr,ls_fetchlong(ls_rxHdr));
            if((rc=ls_zsendhhdr(ZACK,4,ls_txHdr))<0) {
                return rc;
            }
        ...
        case LSZ_TIMEOUT:
            DEBUG(('Z',3,"ls_zinitsender: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
            retransmit = 1;
            break;
        case ZFIN:			/* ZFIN from previous session? Or may be real one? */
            DEBUG(('Z',3,"ls_zinitsender: ZFIN %d",zfins));
            if(++zfins == LSZ_TRUSTZFINS) {
                return LSZ_ERROR;
            }
            break;
        ...
        case ZABORT:		/* Abort this session -- we trust in ABORT! */
            DEBUG(('Z',3,"ls_zinitsender: ABORT"));
            return LSZ_ERROR;
        default:
            /*
            DEBUG(('Z',3,"ls_zinitsender: something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zinitsender: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            break;
    ...
    /*
    DEBUG(('Z',2,"ls_zinitsender: timeout or something other: %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zinitsender: timeout or something other: rc=%ld",rc));
    return rc;
}
...
int ls_zsendfinfo(ZFILEINFO *f, unsigned long sernum, long *pos)
{
    ...
    DEBUG(('Z',2,"ls_zsendfinfo: %s, %d, %d, %d, %d, %d",f->name,f->size,f->mtime,sernum,f->filesleft,f->bytesleft));
    ...
    *p = '\x00';
    ...
    do {
        if(retransmit) {
            DEBUG(('Z',3,"ls_zsendfinfo: resend ZFILE"));
            ...
            if((rc=ls_zsendbhdr(ZFILE,4,ls_txHdr))<0) {
                return rc;
            }
            if((rc=ls_zsenddata(txbuf,finfolen,ZCRCW))<0) {
                return rc;
            }
            retransmit = 0;
            trys++;
        ...
        case ZRPOS:				/* Ok, he want our file */
            DEBUG(('Z',3,"ls_zsendfinfo: ZRPOS at %d",ls_fetchlong(ls_rxHdr)));
        ...
        case ZFERR:		/* Refuse */
            ...
            if(ls_SkipGuard && sn && sn == sernum - 1) {	/* Here is skip protection */
                DEBUG(('Z',3,"ls_zsendfinfo: double-skip protection! for %d, %s (this file: %d, got: %d, ls_SN: %d)",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET],sernum,sn,ls_SerialNum));
                ls_storelong(ls_txHdr,0);
                if((rc=ls_zsendhhdr(ZNAK,4,ls_txHdr))<0) {
                    return rc;
                }
                ...
                DEBUG(('Z',3,"ls_zsendfinfo: turn off double-skip protection in %d, %s (%d)",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET],sn));
                ls_SkipGuard = 0;
            }
        /* Fall through */
        case ZABORT:	/* Abort this session */
        case ZFIN:		/* Finish this session */
            DEBUG(('Z',3,"ls_zsendfinfo: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
            return rc;
        case ZCRC:		/* Send CRC to peer */
            DEBUG(('Z',3,"ls_zsendfinfo: ZCRC for %d bytes",ls_fetchlong(ls_rxHdr)));
            ...
            while((cnt++ < len) && ((c = getc(txfd)) > 0)) {
                crc = LSZ_UPDATE_CRC32(c,crc);
            }
            clearerr(txfd);
            fseek(txfd,0,SEEK_SET);
            crc = LSZ_FINISH_CRC32(crc);
            ls_storelong(ls_txHdr,crc);
            if((rc=ls_zsendhhdr(ZCRC,4,ls_txHdr))<0) {
                return rc;
            }
            break;
        case ZRINIT:
            DEBUG(('Z',3,"ls_zsendfinfo: skip ZRINIT"));
            break;
        case ZNAK:
        case LSZ_TIMEOUT:
            DEBUG(('Z',3,"ls_zsendfinfo: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
        ...
        default:
            /*
            DEBUG(('Z',3,"ls_zsendfinfo: something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zsendfinfo: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            break;
    ...
    /*
    DEBUG(('Z',2,"ls_zsendfinfo: timeout or something other: %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zsendfinfo: timeout or something other: rc=%ld",rc));
    return rc;
}
...
{
    DEBUG(('Z',2,"ls_zrpos: ZRPOS to %d",newpos));
    txpos = newpos;

    if(txpos == ls_txLastRepos) {
        if(++ls_txReposCount > 10) {
            DEBUG(('Z',3,"ls_zrpos: ZRPOS to %ld limit reached",txpos));
            return LSZ_ERROR;
        }
    } else {
        ls_txReposCount = 0;
    ...
    if(fseek(txfd,txpos,SEEK_SET)) {
        DEBUG(('Z',3,"ls_zrpos: ZRPOS to %ld seek error",txpos));
        return LSZ_ERROR;
    }
    if(ls_txCurBlockSize > 32) {
        ls_txCurBlockSize >>= 1;
    }
    ls_txGoodBlocks = 0;
    return LSZ_OK;
...
int ls_zsendfile(ZFILEINFO *f, unsigned long sernum)
{
    ...
    DEBUG(('Z',2,"ls_zsendfile: %s",f->name));

    switch((rc = ls_zsendfinfo(f,sernum,&txpos))) {
    case ZRPOS:		/* Ok, It's OK! */
        DEBUG(('Z',3,"ls_zsendfile: ZRPOS to %d",txpos));
        break;
    case ZSKIP:		/* Skip it */
    case ZFERR:		/* Suspend it */
        DEBUG(('Z',3,"ls_zsendfile: %s",LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
        return rc;
    case ZABORT:
    ...
    default:
        ...
        DEBUG(('Z',3,"ls_zsendfile: Strange answer on ZFILE:  %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
        return LSZ_ERROR;
    }

    if(fseek(txfd,txpos,SEEK_SET)) {
        DEBUG(('Z',3,"ls_zsendfile: Seek to %d error",txpos));
        return LSZ_ERROR;
    }
    sendf.soff=txpos;
    ...
    /* Send file data */
    if(ls_txWinSize) {
        if(ls_rxCould & LSZ_RXCANDUPLEX) {
            mode = sfSlidingWindow;
        } else {
            mode = sfBuffered;
        }
    } else {
    ...
    frame = ZCRCW;
    DEBUG(('Z',2,"ls_zsendfile: mode %s",sfStream==mode?"stream":(sfBuffered==mode?"buffered":"sliding window")));

    while(!feof(txfd)) {
        /* We need to send ZDATA if previous frame was ZCRCW
        Also, frame will be ZCRCW, if it is after RPOS */
        if(ZCRCW == frame) {
            DEBUG(('Z',3,"ls_zsendfile: send ZDATA at %d",txpos));
            ls_storelong(ls_txHdr,txpos);
            if((rc=ls_zsendbhdr(ZDATA,4,ls_txHdr))<0) {
                return rc;
            }
        }
        /* Send frame of data */
        ...
        if(bread < 0) {
            DEBUG(('Z',3,"ls_zsendfile: read error at %d",txpos));
            return LSZ_ERROR;
        }
        /* Select sub-frame type */
        if(bread < ls_txCurBlockSize) {		/* This is last sub-frame -- EOF */
            DEBUG(('Z',3,"ls_zsendfile: EOF at %d",txpos+bread));
            if(sfStream == mode) {
                frame = ZCRCE;
            } else {
                frame = ZCRCW;
            ...
            case sfStream:
                frame = ZCRCG;
                break;	/* Simple sub-frame */
            case sfSlidingWindow:
                frame = ZCRCQ;
                break;	/* Simple sub-frame, but with SlWin */
            case sfBuffered:
                if(txpos + bread > ls_txLastACK + ls_txWinSize) {
                    frame = ZCRCW;    /* Last sub-frame in buffer */
                } else {
                    frame = ZCRCG;    /* Simple sub-frame */
                }
                break;
            }
        }
        DEBUG(('Z',3,"ls_zsendfile: send at %d",txpos));
        if((rc=ls_zsenddata(txbuf,bread,frame))<0) {
            return rc;
        }
        txpos += bread;
        ...
        do {
            ...
            case ZFERR:		/* Problems occured -- suspend file */
                DEBUG(('Z',3,"ls_zsendfile: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                return rc;
            case ZACK:		/* Ok, position ACK */
                DEBUG(('Z',3,"ls_zsendfile: ACK at %d",ls_fetchlong(ls_rxHdr)));
                ls_txLastACK = ls_fetchlong(ls_rxHdr);
                break;
            case ZRPOS:		/* Repos */
                DEBUG(('Z',3,"ls_zsendfile: ZRPOS"));
                if((rc=ls_zrpos(ls_fetchlong(ls_rxHdr)))<0) {
                    return rc;
                }
            ...
            default:		/* STRANGE! */
                /*
                DEBUG(('Z',3,"ls_zsendfile: something strange %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                */
                DEBUG(('Z',3,"ls_zsendfile: something strange rc=%d",rc));
                if(rc < 0) {
                    return rc;
                }
                break;
        ...
        if(trys >= 10) {
            DEBUG(('Z',3,"ls_zsendfile: Trys when waiting for ACK/RPOS exceed"));
            return LSZ_ERROR;
        }
        ...
        if(feof(txfd)) {
            ls_storelong(ls_txHdr,txpos);
            if((rc=ls_zsendhhdr(ZEOF,4,ls_txHdr))<0) {
                return rc;
            }
            trys = 0;
            do {
                ...
                case ZFERR:		/* Problems occured -- suspend file */
                    ...
                    return rc;
                case ZRPOS:		/* Repos */
                    DEBUG(('Z',3,"ls_zsendfile: ZRPOS after ZEOF",ls_fetchlong(ls_rxHdr)));
                    if((rc=ls_zrpos(ls_fetchlong(ls_rxHdr)))<0) {
                        return rc;
                    }
                    frame = ZCRCW; /* Force to retransmit ZDATA */
                ...
                case ZACK:		/* ACK for data -- it lost! */
                    DEBUG(('Z',3,"ls_zsendfile: ACK after EOF at %d",ls_fetchlong(ls_rxHdr)));
                    ls_txLastACK = ls_fetchlong(ls_rxHdr);
                    break;
                case ZABORT:	/* Abort transfer */
                case ZFIN:		/* Strange? Ok, abort too */
                case ZCAN:		/* Abort too */
                    DEBUG(('Z',3,"ls_zsendfile: after ZEOF %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                    return LSZ_ERROR;
                case LSZ_TIMEOUT:	/* Ok, here is no header */
                    DEBUG(('Z',3,"ls_zsendfile: TIEMOUT after ZEOF"));
                    trys++;
                    break;
                default:		/* STRANGE! */
                    DEBUG(('Z',3,"ls_zsendfile: something strange after ZEOF %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                    if(rc<0) {
                        return rc;
                    }
                    trys++;
            ...
            if(feof(txfd)) {
                DEBUG(('Z',3,"ls_zsendfile: To many tries waiting for ZEOF ACK"));
                return LSZ_ERROR;
            }
        }
    }
    /*
    DEBUG(('Z',2,"ls_zsendfile: timeout or something else %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zsendfile: timeout or something else rc=%ld",rc));
    return rc;
}

/* Done sender -- good way */
int ls_zdonesender(void)
{
    int rc;
    int hlen;
    int trys = 0;
    int retransmit = 1;
    DEBUG(('Z',2,"ls_zdonesender"));
    do {
        if(retransmit) {
            ls_storelong(ls_txHdr,0);
            if((rc=ls_zsendhhdr(ZFIN,4,ls_txHdr))<0) {
                return rc;
            }
            trys++;
            retransmit = 0;
        ...
        default:
            /*
            DEBUG(('Z',3,"ls_zdonesender: something strange %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zdonesender: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            retransmit = 1;
    ...
    /*
    DEBUG(('Z',2,"ls_zdonesender: timeout or somethin strange %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zdonesender: timeout or somethin strange rc=%ld",rc));
    return rc;
}
