...
static void freshhelp(void)
{
    ...
    for(i=0; hlp[i]; i++)if(*hlp[i]) {
            if(i%2) {
                wattron(whelp,COLOR_PAIR(8));
            } else {
                wattron(whelp,COLOR_PAIR(9));
            }
            if(*hlp[i]==3&&!slots[currslot]->session) {
                wattron(whelp,(i%2)?COLOR_PAIR(11):(COLOR_PAIR(10)));
            }
            if(*hlp[i]<3&&*hlp[i]!=ins+1) {
                i++;
            } else {
...
static void freshpfile(int b, int e, pfile_t *s, int act)
{
    ...
    wattron(wmain,A_BOLD);
    wmove(wmain,9,b);
    bar(s->toff+s->foff,s->ttot,e-b-9);
    waddch(wmain,' ');
...
static void freshslot(void)
{
    ...
    if(slots[currslot]->chat) {
        ...
        wattrset(wmain,COLOR_PAIR(4));
        mvwaddch(wmain,2,COL-12,' ');
        ...
        mvwhline(wmain,3,0,ACS_HLINE,COL);
        wattron(wmain,A_BOLD);
...
static void logit(char *str,WINDOW *w,int s)
{
    ...
    if(slots[s]->lb[slots[s]->lm]) {
        ...
        if(slots[s]->lm>=LGMAX) {
            ...
            xfree(slots[s]->lb[0]);
            for(len=1; len<LGMAX; len++) {
                slots[s]->lb[len-1]=slots[s]->lb[len];
            }
            slots[s]->lm=LGMAX-1;
...
static void freshqueue(void)
{
    ...
    if(!q) {
        ...
        mvwaddstr(wmain,MH/2,COL/2-8,"* Empty queue *");
            ...
            for(k=0; k<allslots; k++)if(slots[k]->session&&!strcmp(slots[k]->addrs,q->addr)) {
                    k=999;
                }
            mvwaddch(wmain,i+1,0,(k>=999)?'=':' ');
            mvwaddch( wmain, i + 1, 1, current_selector ? '>' : ' ' );
            waddstr(wmain,q->addr);
            for(k=0; k<COL-24-Q_MAXBIT-strlen(q->addr); k++) {
                waddch(wmain,' ');
            }
...
{
    int i;
    for(i=0; i<allslots; i++)if(!strcmp(slots[i]->tty,slt)) {
            return i;
        }
    return -1;
}
...
{
    allslots--;
    if(currslot==allslots||currslot==slt) {
        currslot--;
    }
    ...
    free(slots[slt]);
    for(; slt<=allslots; slt++) {
        slots[slt]=slots[slt+1];
    }
    freshall();
}
...
static int inputstr(char *str, char *name, int mode)
{
    ...
    while(!getkey&&!quitflag) {
        ...
        fr=0;
        if(ch>=32&&ch<255) {
            if(sl>=(mode?(mode==1?40:5):(MAX_STRING-2))||(mode&&!strchr(mode==1?"icdhny:/.@1234567890 ":"1234567890",ch))) {
                xbeep();
            } else {
                if(ins)for(i=sl; i>=sp; i--) {
                        str[i+1]=str[i];
                    }
                str[sp]=(char)ch;
                sp++;
            ...
            case KEY_UP:
                if(hstlast&&hstcurr) {
                    ...
                    tmp=hstcurr;
                    hstcurr--;
                    while(hstcurr&&(*hst[hstcurr]!=mode+1)) {
                        hstcurr--;
                    }
            ...
            case KEY_DOWN:
                if(hstcurr<hstlast) {
                    hstcurr++;
                    while(hstcurr<hstlast&&hst[hstcurr]&&(*hst[hstcurr]!=mode+1)) {
                        hstcurr++;
                    }
                    if(hst[hstcurr]) {
                        strcpy(str,hst[hstcurr]+1);
...
rei:
    zone=strtoul(myaddr,&nm,10);
    net=strtoul(nm+1,&nm,10);
    node=strtoul(nm+1,&nm,10);
    if(*nm=='.') {
        point=strtoul(nm+1,NULL,10);
    }
    inputstr(buf,name,1);
    *(long*)ou=0;
    flv='n';
    if(*buf>32) {
        ...
        if(point) {
            snprintf(ou+strlen(ou),63 - strlen(ou),".%lu",point);
        }
        if(flv=='e') {
            ...
            *(int*)ou=0;
            ...
            write_log("in: '%s', ident as '%s'",buf,ou);
        }
#endif
    }
    return ou;
...
#endif
    if(xsend(sock,buf,len)<0) {
        write_log("can't send to server: %s",strerror(errno));
    }
}
...
static int getmessages(char *bbx)
{
...
#if QDEBUG==1
    if(rc>0) {
        write_log("recv: [%d,%d] '%s','%s'",buf[2],rc,buf+3,buf+4+strlen(buf+3));
    }
#endif
    if(bbx&&rc>=3&&buf[2]<8) {
        memcpy(bbx,buf,rc);
    ...
    if(rc>=3) {
        ...
        if(strcmp(buf+3,"master")) {
            if(!strcasecmp(buf+3,"ipline")) {
                snprintf(buf+5,5,"%04x",id);
            }
            rc=findslot(buf+3);
            if(type==QC_ERASE) {
            ...
            case QC_EMSID:
                if(!len) {
                    ...
                    p=data;
                    slots[rc]->speed=FETCH32(p);
                    INC32(p);
                    slots[rc]->opt=FETCH16(p);
                    INC16(p);
                    slots[rc]->options=FETCH32(p);
                    INC32(p);
                    slots[rc]->start=FETCH32(p);
                    INC32(p);
                    restrcpy(&slots[rc]->name,(char*)p);
                    p+=strlen((char*)p)+1;
                    restrcpy(&slots[rc]->sysop,(char*)p);
                    p+=strlen((char*)p)+1;
                    restrcpy(&slots[rc]->city,(char*)p);
                    p+=strlen((char*)p)+1;
                    restrcpy(&slots[rc]->flags,(char*)p);
                    p+=strlen((char*)p)+1;
                    restrcpy(&slots[rc]->phone,(char*)p);
                    p+=strlen((char*)p)+1;
            ...
            case QC_SENDD:
                if(!len) {
                    ...
                    slots[rc]->s.foff=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.ftot=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.toff=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.ttot=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.nf=FETCH16(p);
                    INC16(p);
                    slots[rc]->s.allf=FETCH16(p);
                    INC16(p);
                    slots[rc]->s.cps=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.soff=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.stot=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.start=FETCH32(p);
                    INC32(p);
                    slots[rc]->s.mtime=FETCH32(p);
                    INC32(p);
            ...
            case QC_RECVD:
                if(!len) {
                    ...
                    slots[rc]->r.foff=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.ftot=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.toff=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.ttot=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.nf=FETCH16(p);
                    INC16(p);
                    slots[rc]->r.allf=FETCH16(p);
                    INC16(p);
                    slots[rc]->r.cps=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.soff=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.stot=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.start=FETCH32(p);
                    INC32(p);
                    slots[rc]->r.mtime=FETCH32(p);
                    INC32(p);
    ...
    while(!quitflag) {
        ...
        if (sizechanged) {
            if(!ioctl(fileno(stdout),TIOCGWINSZ,&size)) {
                ...
                draw_screen();
                for(ch=0; ch<allslots; ch++) {
                    wresize(slots[ch]->wlog,LOGSIZE,COL);
                }
                wresize(wmain,MH,COL);
                wresize(wlog,LOGSIZE,COL);
                mvwin(wstat,LINES-2,2);
                wresize(wstat,1,COL-2);
                mvwin(whelp,LINES-1,1);
                wresize(whelp,1,COL);
                wresize(whdr,1,COL-2);
                freshall();
            }
            sizechanged=0;
        }
        ...
        if(rc<0&&errno!=EINTR) {
            write_log("err in select: %s",strerror(errno));
        }
        ...
        if(rc>0&&FD_ISSET(0,&rfds)) {
            ...
            if(ch==('L'-'@')) {
                ...
                if(ch!=KEY_F(2)) {
                    ...
                    if(ch=='\b') {
                        waddstr(slots[currslot]->wlog,"\b ");
                    }
                    if(ch) {
                        waddch(slots[currslot]->wlog,ch);
                    } else {
                        waddstr(slots[currslot]->wlog,"\nClosing..\n");
                    }
                ...
                if ( ch == '`' || ( ch >= '0' && ch <= '9' )) {
                    ...
                    if(currslot<0)switch(ch) {
                        case KEY_UP:
                            ...
                            break;
                        case KEY_DOWN:
                            if(q_pos<q_max)if(++q_pos>=q_first+MH-1) {
                                    q_first++;
                                }
                            freshqueue();
                            wrefresh(wmain);
                        ...
                        case 'I':
                        case 'h':
                            for(; que&&que->n!=q_pos; que=que->next);
                            if(!que) {
                                break;
                            }
                        ...
                        case 'S':
                            ...
                            *bf++=0;
                            if(inputstr(bf,"Full file name: ",0)) {
                                if(access(bf,R_OK)==-1) {
                                    write_log("warn: can't access file '%s'",bf);
                                }
                                while(*bf++)if(*bf==' ') {
                                        *bf='?';
                                    }
