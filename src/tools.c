...
static int initcharset(char *name,unsigned char *tab)
{
    ...
    while(fgets(buf,MAX_STRING,f)) {
        ...
        if(*buf=='0'&&buf[1]=='x') {
            if(sscanf(buf,"0x%x 0x%x",&i,&c)!=2) {
                continue;
            }
        } else {
            if(sscanf(buf,"%u %u",&i,&c)!=2) {
                continue;
            }
        }
        if(rev) {
        ...
        if(c>255||i<128||i>255) {
            continue;
        }
        tab[i-128]=c;
    }
    for(n=0; n<128; n++)if(!tab[n]) {
            tab[n]=n+128;
        }
    fclose(f);
    return 1;
...
void recode_to_remote(char *s)
{
    ...
    if(!loaded||!s) {
        loaded=initcharset(cfgs(CFG_REMOTECP),ctab_remote);
    }
    if(loaded==1&&s) {
        while(*s) {
            c=*(unsigned char*)s;
            if(c>=128) {
                *(unsigned char*)s=ctab_remote[c-128];
            }
            s++;
        }
    }
...
void recode_to_local(char *s)
{
    ...
    register unsigned char c;
    if(!loaded||!s) {
        loaded=initcharset(cfgs(CFG_LOCALCP),ctab_local);
    }
    if(loaded==1&&s) {
        while(*s) {
            c=*(unsigned char*)s;
            if(c>=128) {
                *(unsigned char*)s=ctab_local[c-128];
            }
            s++;
        }
    }
