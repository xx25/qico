...
al_item_t *ipline_alloc(void *key)
{
    ...
    if(!(p->clients=al_new(lien2ui_alloc,line2ui_free,line2ui_cmp))) {
        return NULL;
    }
    return (al_item_t*)p;
}
...
void anyline_free(al_item_t *i)
{
    ...
    for(i=0; i<LOG_BUFFER_SIZE; i++) if(p->log_buffer[i]) {
            xfree(p->log_buffer[i]);
        }
    if(p->clients) {
        al_free(p->clients);
    }
...
static int set_all_opts(int s)
{
    ...
    if(!setsockopt(s,SOL_SOCKET,SO_REUSEPORT,&x,sizeof(x))) {
        return -1;
    }
...
int server_init()
{
    ...
    if(!(ip_lines=al_new(ipline_alloc,anyline_free,ipline_cmp))) {
        return -1;
    }
    /* Allocate list fot TTY lines */
    if(!(tty_lines=al_new(ttyline_alloc,ttyline_free,ttyline_cmp))) {
        return -1;
    }
    /* Allocate list for clients */
    if(!(clients=al_new(uiclient_alloc,uiclient_free,uiclient_cmp))) {
        return -1;
    }
    ...
    if(!set_all_opts(line_socket)) {
        return -1;
    }
    if(bind(line_socket,(struct sockaddr*)&sin,sizeof(sin))) {
        return -1;
    }
    ...
    if(!set_all_opts(client_socket)) {
        return -1;
    }
    if(bind(client_socket,(struct sockaddr*)&sin,sizeof(sin))) {
        return -1;
    }
    if(listen(client_socket,4)) {
        return -1;
    }
...
int server_process_line_event(linestate_t *line, evtlam_t *evt, struct sockaddr_in *sa)
{
    ...
    if(line->pid && line->pid != evt->pid) {
        ...
        if(!evt->tty[0]) {
            al_deli(ip_lines,(al_item_t*)line);
        } else {
            ttyline_clear(line);
        }
        return -1;
    ...
    case EVTL2M_BATCH_START:				/* Batch starts */
        ...
        if(!(txrx = get_txrx_state(line,c1))) {
            return -1;
        }
    ...
    do {
        ...
        if(FD_ISSET(rfds,line_socket)) {
            ...
            if(uic) {
                ...
                if(uic->socket != -1) {
                    al_toend(clients,(al_item_t*)uic);
                } else {
                    xfree(uic);
                }
            }
