...
static XS(perl_wlog)
{
    ...
    if(items==1||items==2) {
        ...
        if(!lvl) {
            ...
            DEBUG(('P',lvl,"%s",len?str:NULL));
        }
    } else {
        write_log("perl wlog() error: wrong number of args");
    }
    XSRETURN_EMPTY;
}
...
static XS(perl_setflag)
{
    ...
    if(items==2) {
        ...
        if(num<0||num>9) {
            write_log("perl setflags() error: illegal argument");
        } else {
            if(arg) {
                perl_flg|=1<<num;
            } else {
        ...
        write_log("perl setflags() error: wrong number of args");
    }
    XSRETURN_EMPTY;
}
...
static XS(perl_qexpr)
{
    ...
    if(items==1) {
        ...
        write_log("perl qexpr() error: wrong number of args");
    }
    XSRETURN_IV(rc);
}
...
int perl_init(const char *script, int mode)
{
    ...
    for(rc=0,perl_nc=0; rc<sizeof(perl_subnames)/sizeof(*perl_subnames); rc++)
        if(perl_get_cv(perl_subnames[rc],FALSE)) {
            perl_nc|=1<<rc;
        }
    if(!perl_nc) {
        perl_done(0);
