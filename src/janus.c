...
static byte rcvpkt(void)
{
    ...
    /*-------------------------------------------------------------------------*/
    while((c=rcvbyte(1))>=0 && p<rxbufmax) {
        *p++=c;
    }
...
static void getfname(flist_t **l)
{
    ...
    if(txfd) {
        snprintf((char *)txbuf,1024,"%s%c%lu %lo %o%c%c",(*l)->sendas,0,(long)sendf.ftot,sendf.mtime,0644,0,OUR_JCAPS);
    } else {
        snprintf((char *)txbuf,1024,"%c%c%c",0,0,OUR_JCAPS);
    }
}
