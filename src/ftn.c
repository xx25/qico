...
int parseftnaddr(const char *s, ftnaddr_t *a, const ftnaddr_t *b, int wc)
{
    ...
    if(b) {
        ...
        a->z=a->n=a->f=a->p=wc?-1:0,a->d=NULL;
    }
    for(pn=p; *p&&pq; p++) {
        if(isdigit((int)*p)) {
            wn=1;
...
char *ftnaddrtoa(const ftnaddr_t *a)
{
    static char s[30];
    if(a->p) {
        snprintf(s,30,"%d:%d/%d.%d",a->z,a->n,a->f,a->p);
    } else {
        snprintf(s,30,"%d:%d/%d",a->z,a->n,a->f);
    }
    return s;
}
...
char *ftnaddrtoda(const ftnaddr_t *a)
{
    ...
    if(a->p) {
        snprintf(s,64,"%d:%d/%d.%d@%s",a->z,a->n,a->f,a->p,a->d?a->d:d);
    } else {
        snprintf(s,64,"%d:%d/%d@%s",a->z,a->n,a->f,a->d?a->d:d);
    }
    return s;
}
...
char *ftnaddrtoia(const ftnaddr_t *a)
{
    static char s[64];
    if(a->p) {
        snprintf(s,64,"p%d.f%d.n%d.z%d." FTNDOMAIN,a->p,a->f,a->n,a->z);
    } else {
        snprintf(s,64,"f%d.n%d.z%d." FTNDOMAIN,a->f,a->n,a->z);
    }
    return s;
}
                 ...
                 char *from, char *to, char *subj, char *pwd, char *fn, unsigned attr)
{
    ...
    ph.phCaps=H2I16(1);
    if(pwd) {
        memcpy(ph.phPass,pwd,MAX(strlen(pwd),8));
    }
    ...
    if(fa->p) {
        fprintf(f,"\001FMPT %d\r",fa->p);
    }
    if(ta->p) {
        fprintf(f,"\001TOPT %d\r",ta->p);
    }
...
int needhold(int status, int what)
{
    ...
    if (( status & Q_WAITR ) && !( what & (~T_REQ))) {
        return 1;
    }
    if (( status & Q_WAITX ) && !( what & (~T_ARCMAIL))) {
        return 1;
    }
    return 0;
}
