...
static char *sts_str(int flags)
{
    ...
    char qchars[]=Q_CHARS;
    int i;
    for(i=0; i<Q_MAXBIT; i++) {
        s[i]=(flags&(1<<i))?qchars[i]:'.';
    }
    s[Q_MAXBIT]=0;
    return s;
}
...
int daemon_xsend(int sock, const char *buf, size_t len)
{
    ...
    for(uis=cl; uis; uis=uis->next)
        if(uis->type=='e'&&(!tosend||tosend==uis->sock))
            if(xsend(uis->sock,buf,len)<0) {
                DEBUG(('I',1,"can't send to client %d (fd=%d): %s",uis->id,uis->sock,strerror(errno)));
            }
    return len;
}
...
static void daemon_evt(int chld,char *buf,int rc,int mode)
{
    ...
    if(!mode) {
        if(buf[2]==QC_EMSID||buf[2]==QC_LIDLE) {
            ...
            if(lins) {
                if(rc>4&&buf[2]==QC_EMSID) {
                    lins->emsi=xrealloc(lins->emsi,rc);
                } else {
                    xfree(lins->emsi);
                }
                DEBUG(('I',4,"got emsi (%d)",lins->emsi?rc:0));
                if(lins->emsi) {
                    memcpy(lins->emsi,buf,rc);
                }
                lins->emsilen=rc;
            } else {
                DEBUG(('I',1,"got emsi from unknown line %d",FETCH16(buf)));
            }
        }
        if(rc>2) {
    ...
    if(buf[2]==QR_STYPE) {
        ...
        if(uis->type=='e') {
            ...
            for(lins=ln; lins; lins=lins->next)
                if(lins->emsi) {
                    daemon_xsend(chld,lins->emsi,lins->emsilen);
                }
            tosend=0;
        }
        return;
    ...
    if(FETCH16(buf)) {
        ...
        for(lins=ln; lins&&lins->id!=FETCH16(buf); lins=lins->next);
        if(lins) {
            if(xsendto(lins_sock,buf,rc,lins->addr)<0) {
                DEBUG(('I',1,"can't send to line %d: %s",lins->id,strerror(errno)));
            }
        } else {
            DEBUG(('I',2,"line not found"));
        }
        return;
    }
    ...
    case QR_STS:
        ...
        if(rc) {
            ...
            if(rc!=2) {
                ...
                write_log("hold %s for %d min (new status: [%s])",ftnaddrtoa(&fa),FETCH16(p),sts_str(sts.flags));
            }
            if(set&Q_WAITA&&!(res&Q_ANYWAIT)) {
                sts.htime=timer_set((rc==2?(FETCH16(p)):cfgi(CFG_WAITHRQ))*60);
            }
            if(set&Q_UNDIAL) {
            ...
            while(!dable&&i) {
                ...
                if ( checktimegaps( cfgs( CFG_CANCALL ))
                    ...
                    if(!chld) {
...
#endif
                        if(!outbound_locknode(&current->addr,LCK_c)) {
                            exit(S_BUSY);
                        }
                        if(!is_ip&&cfgi(CFG_TRANSLATESUBST)) {
                            phonetrans(&rnode->phone,cfgsl(CFG_PHONETR));
                        }
                        log_done();
                        ssock=uis_sock=lins_sock=-1;
                        log_callback=NULL;
                        xsend_cb=NULL;
                        ...
                        hld=0;
                        if(!log_init(cfgs(CFG_MASTERLOG),NULL)) {
                            write_log("can't init master log %s",ccs);
                        }
                        ...
                        if(rc!=S_BUSY) {
                            t_rescan=cfgi(CFG_RESCANPERIOD)-1;
                        }
                        switch(rc&S_MASK) {
                        case S_BUSY:
                            break;
        ...
        while(do_rescan!=1&&timer_time(t)<1) {
            ...
            if(rc>0) {
                if(FD_ISSET(lins_sock,&rfds)) {
                    ...
                    /* nmap -sU >>> */		  if(recvfrom(lins_sock,buf,2,MSG_PEEK,&sa,&salen)<2) {
                        DEBUG(('I',1,"recvfrom: error: %s",strerror(errno)));
                    } else {
                        rc=xrecv(lins_sock,buf,MSG_BUFFER-1,1);
                        if(rc<0) {
                            DEBUG(('I',3,"xrecv_l: %s",strerror(errno)));
                        }
                        if(rc>0) {
                ...
                for(uis=cl,uit=NULL; uis; uit=uis,uis=uis?uis->next:NULL)
                    if(FD_ISSET(uis->sock,&rfds)) {
                        ...
                        if(rc<0&&errno!=EAGAIN) {
                            DEBUG(('I',3,"xrecv_u %d: %s",uis->id,strerror(errno)));
                        }
                        if(rc>0) {
                            DEBUG(('I',8,"client %d: recv %d bytes",uis->id,rc));
                        }
                        if(rc>1) {
                            daemon_evt(uis->sock,buf,rc,1);
                        }
                    }
