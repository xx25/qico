/* MD5Q.C - RSA Data Security, Inc., MD5 message-digest algorithm
 */
...
#else
#define Encode memmove
#define Decode memmove
#define MD5_memcpy memmove
#endif
...
md_caddr_t      digest;              /* caller digest to be filled in */

{
    ...
    MD5_memset( k_ipad + key_len, 0, (sizeof k_ipad) - key_len);
    MD5_memset( k_opad + key_len, 0, (sizeof k_opad) - key_len);
...
unsigned char *md5_challenge(const unsigned char *buf)
{
    unsigned char *ret = NULL;

    if ( buf == NULL ) {		/* Make new challenge */
        md_caddr_t digest;
        ret = (unsigned char *) xcalloc( 1, MD_CHALLENGE_LEN + 1 );

        STORE32( ret, time( NULL ));
        STORE32( ret + 4, random() );
        STORE32( ret + 8, getpid() ^ random() );
        STORE32( ret + 12, (( random() << 4 ) + ( time( NULL ) & 0xff )));
        STORE32( digest, getpid() );
        hmac_md5( ret, strlen((char *) ret ), digest, MD5_DIGEST_LEN, digest );
        memcpy( ret + 1, digest, MD5_DIGEST_LEN );
        *ret = MD5_DIGEST_LEN;
    } else {				/* Decode challenge */
        char *n;
        int count;
        ...
        *ret = strhex2bin( ret + 1, (unsigned char *) n );
    }
    return ret;
}


char *md5_digest(const char *pwd, const unsigned char *challenge)
{
    md_caddr_t digest;
    char *ret = NULL;
