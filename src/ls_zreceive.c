...
int ls_zinitreceiver(int protocol, int baud, int window, ZFILEINFO *f)
{
    ...
    DEBUG(('Z',2,"ls_zinitreceiver: %08x, %d baud, %d bytes",protocol,baud,window));
...
int ls_zrecvcrcw(byte *buf, int *len)
{
    ...
    DEBUG(('Z',2,"ls_zrecvcrcw"));
    do {
        switch((rc=ls_zrecvdata(buf,len,ls_DataTimeout,ls_Protocol&LSZ_OPTCRC32))) {
        case ZCRCW:	/* Ok, here it is */
            DEBUG(('Z',3,"ls_zrecvcrcw: got frame ZCRCW"));
            return LSZ_OK;
        case LSZ_BADCRC:
            DEBUG(('Z',3,"ls_zrecvcrcw: BADCRC"));
            ls_storelong(ls_txHdr,0L);
            ls_zsendhhdr(ZNAK,4,ls_txHdr);
            return 1;
        case LSZ_TIMEOUT:
            DEBUG(('Z',3,"ls_zrecvcrcw: TIMEOUT"));
            break;
        default:
            DEBUG(('Z',3,"ls_zrecvcrcw:: strange data frame %d",rc));
            if(rc<0) {
                return rc;
            }
            ls_storelong(ls_txHdr,0L);
...
int ls_zrecvfinfo(ZFILEINFO *f, int frame, int first)
{
    ...
    DEBUG(('Z',2,"ls_zrecvfinfo: frame %d, %s, first: %d",frame,LSZ_FRAMETYPES[frame+LSZ_FTOFFSET],first));
    ...
    if(ls_Protocol&LSZ_OPTESCAPEALL) {
        flags |= LSZ_RXWNTESCCTL;
    }
    if(ls_Protocol&LSZ_OPTESC8) {
        flags |= LSZ_RXWNTESC8;
    }
    ...
    do {
        if(retransmit) {
            if(ZRINIT != frame) {
                DEBUG(('Z',3,"ls_zrecvfinfo: send %d, %s",frame,LSZ_FRAMETYPES[frame+LSZ_FTOFFSET]));
                ls_storelong(ls_txHdr,ls_SerialNum);
                if((rc=ls_zsendhhdr(frame,4,ls_txHdr))<0) {
                    return rc;
                }
            }
            DEBUG(('Z',3,"ls_zrecvfinfo: send %d, ZRINIT",ZRINIT));
            ...
            if((rc=ls_zsendhhdr(ZRINIT,4,ls_txHdr))<0) {
                return rc;
            }
            retransmit = 0;
            trys++;
        ...
        case ZNAK:
        case LSZ_TIMEOUT:
            DEBUG(('Z',3,"ls_zrecvfinfo: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
            retransmit = 1;
            break;
        case ZSINIT:		/* He want to set some options */
            DEBUG(('Z',3,"ls_zrecvfinfo: ZSINIT"));
            first = 0;      /* We will trust in first ZFIN after ZSINIT */
            if((rc=ls_zrecvcrcw(rxbuf,&len))<0) {
                return rc;
            }
            if(!rc) { 		/* Everything is OK */
                ...
                if(ls_rxHdr[LSZ_F0]&LSZ_TXWNTESCCTL) {
                    ls_Protocol |= LSZ_OPTESCAPEALL;
                }
                if(ls_rxHdr[LSZ_F0]&LSZ_TXWNTESC8) {
                    ls_Protocol |= LSZ_OPTESC8;
                }
        ...
        case ZFILE:			/* Ok, File started! */
            ...
            if(!rc) { 		/* Everything is OK, decode frame */
                ...
                if(sscanf((char *)rxbuf+strlen(f->name)+1,"%ld %lo %o %o %ld %ld",(long*)&f->size,(unsigned long*)&f->mtime,(unsigned*)&len,(unsigned*)&ls_SerialNum,(long*)&f->filesleft,(long*)&f->bytesleft) < 2) {
                    DEBUG(('Z',4,"ls_zrecvfinfo: file info is corrupted: '%s'",rxbuf+strlen(f->name)+1));
                    f->filesleft = -1;
                }
                return ZFILE;
            } else {		/* We could not receive ZCRCW subframe, but error is not fatal */
        ...
        case ZFIN:			/* ZFIN from previous session? Or may be real one? */
            DEBUG(('Z',3,"ls_zrecvfinfo: ZFIN %d, first: %d",zfins,first));
            if(first || ++zfins == LSZ_TRUSTZFINS) {
                return ZFIN;
            }
            break;
        case ZABORT:		/* Abort this session -- we trust in ABORT! */
            DEBUG(('Z',3,"ls_zrecvfinfo: ABORT"));
            return ZABORT;
        case LSZ_BADCRC:
            DEBUG(('Z',3,"ls_zrecvfinfo: BADCRC"));
            ls_storelong(ls_txHdr,0L);
            ls_zsendhhdr(ZNAK,4,ls_txHdr);
            retransmit = 1;
            break;
        default:
            /*
            DEBUG(('Z',3,"ls_zrecvfinfo: something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zrecvfinfo: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            ls_storelong(ls_txHdr,0L);
    ...
    /*
    DEBUG(('Z',2,"ls_zrecvfinfo: timeout or something other: %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zrecvfinfo: timeout or something other: rc=%ld",rc));
    return LSZ_TIMEOUT;
}
...
int ls_zrecvnewpos(unsigned long oldpos, unsigned long *pos)
{
    ...
    DEBUG(('Z',2,"ls_zrecvnewpos: will retransmit %d",oldpos));
    do {
        switch((rc=ls_zrecvhdr(ls_rxHdr,&hlen,ls_HeaderTimeout))) {
        case ZDATA:
        case ZEOF:
            DEBUG(('Z',3,"ls_zrecvnewpos: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
            *pos = ls_fetchlong(ls_rxHdr);
            return rc;
        case ZNAK:
            DEBUG(('Z',3,"ls_zrecvnewpos: ZNAK"));
            ls_storelong(ls_txHdr,oldpos);
            if((rc=ls_zsendhhdr(ZRPOS,4,ls_txHdr))<0) {
                return rc;
            }
            break;
        case LSZ_TIMEOUT:
            DEBUG(('Z',3,"ls_zrecvnewpos: TIEMOUT"));
            break;
        case LSZ_BADCRC:
            DEBUG(('Z',3,"ls_zrecvnewpos: BADCRC"));
            ls_storelong(ls_txHdr,0L);
            ls_zsendhhdr(ZNAK,4,ls_txHdr);
            break;
        default:
            /*
            DEBUG(('Z',3,"ls_zrecvnewpos: something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                                 */
            DEBUG(('Z',3,"ls_zrecvnewpos: something strange rc=%ld",rc));
            if(rc<0) {
                return rc;
            }
            ls_storelong(ls_txHdr,0L);
    ...
    /*
    DEBUG(('Z',2,"ls_zrecvnewpos: timeout or something strange %ld, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
           */
    DEBUG(('Z',2,"ls_zrecvnewpos: timeout or something strange rc=%ld",rc));
    return LSZ_TIMEOUT;
}
...
int ls_zrecvfile(int pos)
{
    ...
    DEBUG(('Z',2,"ls_zrecvfile form %d",pos));
    ...
    do {
        if(!needzdata) {
            ...
            case ZCRCE:
                needzdata = 1;
            case ZCRCG:
                DEBUG(('Z',3,"ls_zrecvfile: ZCRC%c, %d bytes at %d",rc==ZCRCE?'E':'G',len,rxpos));
                rxpos += len;
                if(len != fwrite(rxbuf,1,len,rxfd)) {
                    return ZFERR;
                }
            ...
            case ZCRCW:
                needzdata = 1;
            case ZCRCQ:
                DEBUG(('Z',3,"ls_zrecvfile: ZCRC%c, %d bytes at %d",rc==ZCRCW?'W':'Q',len,rxpos));
                rxpos += len;
                if(len != fwrite(rxbuf,1,len,rxfd)) {
                    return ZFERR;
                }
            ...
            case LSZ_BADCRC:
            case LSZ_TIMEOUT:
                DEBUG(('Z',3,"ls_zrecvfile: %d, %s",rc,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                if(ls_rxAttnStr[0]) {
                    PUTSTR(ls_rxAttnStr);
                }
                PURGE();
                ...
                break;
            default:
                DEBUG(('Z',3,"ls_zrecvfile: something strange %ld",rc));
                if(rc<0) {
                    return rc;
                }
                if(ls_rxAttnStr[0]) {
            ...
            DEBUG(('Z',3,"ls_zrecvfile: want ZDATA/ZEOF at %d",rxpos));
            if((rc=ls_zrecvnewpos(rxpos,&newpos))<0) {
                return rc;
            }
            if(newpos != rxpos) {
                DEBUG(('Z',4,"ls_zrecvfile: bad new position %d in %s",newpos,LSZ_FRAMETYPES[rc+LSZ_FTOFFSET]));
                if(ls_rxAttnStr[0]) {
                    PUTSTR(ls_rxAttnStr);
                }
                PURGE();
                ...
                DEBUG(('Z',4,"ls_zrecvfile: ZDATA"));
                needzdata = 0;
            }
        }
        if(rxstatus) {
            return((rxstatus==RX_SKIP)?ZSKIP:ZFERR);
        }
    } while(1);
    return LSZ_OK;
}
...
int ls_zdonereceiver(void)
{
    ...
    do {
        ...
        case XON | 0x80:
        case XOFF | 0x80:
            DEBUG(('Z',3,"ls_zdonereceiver: XON/XOFF, skip it"));
            break;
        case ZPAD:
            DEBUG(('Z',3,"ls_zdonereceiver: ZPAD"));
            if((rc=ls_zrecvhdr(ls_rxHdr,&hlen,ls_HeaderTimeout))<0) {
                return rc;
            }
            if(ZFIN != rc) {
                return LSZ_OK;
            ...
            break;
        default:
            DEBUG(('Z',3,"ls_zdonereceiver: something strange %d",rc));
            if(rc<0) {
                return rc;
            }
            retransmit = 1;
    ...
    DEBUG(('Z',2,"ls_zdonereceiver: timeout or something strange %d",rc));
    return rc;
}
