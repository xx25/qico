...
{
    if(_Z(q1)->addr.z!=_Z(q2)->addr.z) {
        return _Z(q1)->addr.z-_Z(q2)->addr.z;
    }
    if(_Z(q1)->addr.n!=_Z(q2)->addr.n) {
        return _Z(q1)->addr.n-_Z(q2)->addr.n;
    }
    if(_Z(q1)->addr.f!=_Z(q2)->addr.f) {
        return _Z(q1)->addr.f-_Z(q2)->addr.f;
    }
    ...
    if(cfgs(CFG_LONGBOXPATH)) {
        if((d=opendir(ccs))!=0) {
            while((de=readdir(d))) {
                ...
                if(n) {
                    DEBUG(('Q',4,"found lbox '%s', parse: %d:%d/%d.%d (%c) rc=%d",de->d_name,a.z,a.n,a.f,a.p,n==5?flv:'*',n));
                }
                if(n==4||n==5) {
                    if(n==4) {
                        snprintf(rev,40,"%d.%d.%d.%d",a.z,a.n,a.f,a.p);
                    } else {
                        snprintf(rev,40,"%d.%d.%d.%d.%c",a.z,a.n,a.f,a.p,flv);
                    }
                    if(!strcmp(de->d_name,rev)) {
                        ...
                        if(q->sizes[5]!=0) {
                            ...
                            case 'i':
                                ...
                                break;
                            default :
                                write_log("unknown longbox flavour '%c' for dir %s",flv,de->d_name);
                            }
                        }
                        xfree(p);
                    } else {
                        DEBUG(('Q',1,"strange: find: '%s', calc: '%s'",de->d_name,rev));
                    }
                } else if(*de->d_name!='.') {
                    DEBUG(('Q',2,"bad longbox name %s",de->d_name));
                }
            }
            closedir(d);
        } else {
            write_log("can't open %s: %s",ccs,strerror(errno));
        }
    }
}
