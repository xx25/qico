...
static int sifname(char *s)
{
    ...
    int i;
    for(i = 0; i < CHARS; i++) if(*s < ALLOWED_CHARS[i]) {
            *s = ALLOWED_CHARS[i];
            break;
        }
    if(i == CHARS) {
...
int rxopen(char *name, time_t rtime, off_t rsize, FILE **f)
{
    ...
    recvf.ftot=rsize;
    if(recvf.toff+rsize > recvf.ttot) {
        recvf.ttot+=rsize;
    }
    recvf.nf++;
    if(recvf.nf>recvf.allf) {
        recvf.allf++;
    }
    IFPerl(if((rc=perl_on_recv())!=FOP_OK)return rc);
    if(whattype(name)==IS_PKT&&(rsize==60||!rsize)&&cfgi(CFG_KILLBADPKT)) {
        return FOP_SKIP;
    }
    rc=skipiftic;
    skipiftic=0;
...
int rxclose(FILE **f, int what)
{
    ...
    recvf.toff+=recvf.foff;
    recvf.stot+=recvf.soff;
    *p2=0;
    if(!cps) {
        cps=1;
    }
    cps=(recvf.foff-recvf.soff)/cps;
    IFPerl(if((ss=perl_end_recv(what))) {
    if(!*ss) {
            what=FOP_SKIP;
        } else {
            xstrcpy(p2,ss,MAX_PATH);
        }
    });
    switch(what) {
    case FOP_SUSPEND:
    ...
    *f=NULL;
    ...
    case FOP_SUSPEND:
    case FOP_ERROR:
        if(whattype(p)==IS_PKT&&cfgi(CFG_KILLBADPKT)) {
            lunlink(p);
        } else {
            utime(p,&ut);
        }
    ...
    if(what==FOP_SKIP||what==FOP_SUSPEND) {
        skipiftic=what;
    }
    recvf.start=0;
    recvf.ftot=0;
...
FILE *txopen(char *tosend, char *sendas)
{
    ...
    if(whattype(sendas)==IS_PKT&&sb.st_size==60) {
        return NULL;
    }
    xfree(sendf.fname);
    ...
    if(sendf.toff+sb.st_size > sendf.ttot) {
        sendf.ttot+=sb.st_size;
    }
    sendf.nf++;
    if(sendf.nf>sendf.allf) {
        sendf.allf++;
    }
...
int txclose(FILE **f, int what)
{
    ...
    sendf.toff+=sendf.foff;
    sendf.stot+=sendf.soff;

    if(!cps) {
        cps=1;
    }
    cps=(sendf.foff-sendf.soff)/cps;
...
int chatsend(unsigned char *str)
{
    ...
    if(chattimer<2) {
        ...
        if(!c_devsend(ubuf,strlen((char*)ubuf))) {
            return 1;
        }
        if(chatlg) {
            chatlog_write((char*)str,0);
        }
    } else {
        write_log("Chat already opened!");
    }
...
void c_devrecv(unsigned char *data,unsigned len)
{
    ...
    if(chattimer<2) {
        char *p;
        if(len>5&&data[1]!=5&&(p=strstr((char*)data," * "))&&(strstr(p+3,"lose")||strstr(p+3,"has chat"))) {
            return;
        }
    ...
    if(data[strlen((char*)data)-1]==5) {
        data[strlen((char*)data)-1]='\n';
    }
    for(len=i=0; len<=strlen((char*)data); len++)if(data[len]!=5) {
            data[i++]=data[len];
        }
    chattimer=time(NULL)+TIM_CHAT;
    if(chatlg) {
        chatlog_write((char*)data,1);
    }
    qchat((char*)data);
}
...
void getevt(void)
{
    ...
    while(qrecvpkt((char*)qrcv_buf)) {
        ...
        case QR_CHAT:
            if(qrcv_buf[3]) {
                ...
                if(qsndbuflen>CHAT_BUF-128) {
                    qsndbuflen=CHAT_BUF-128;
                }
            } else {
                i=chatprot;
                chatprot=-1;
    ...
    if(qsndbuflen>0)if(!chatsend(qsnd_buf)) {
            qsndbuflen=0;
        }
}
...
void check_cps(void)
{
    ...
    if(!(sendf.cps=time(NULL)-sendf.start)) {
        sendf.cps=1;
    } else {
        sendf.cps=(sendf.foff-sendf.soff)/sendf.cps;
    }
    if(!(recvf.cps=time(NULL)-recvf.start)) {
        recvf.cps=1;
    } else {
